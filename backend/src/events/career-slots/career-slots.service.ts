...
export class CareerSlotsService {
  ...
  async listSlots(eventId: string, callerUid: string): Promise<CareerSlotWithMeta[]> {
    const slots = await this.careerSlotsRepository.listSlots(eventId);

    const [creators, approvedCounts, myRequests] = await Promise.all([
      Promise.all(slots.map((s) => this.usersRepository.findByUid(s.createdByUid))),
      Promise.all(slots.map((s) => this.careerSlotsRepository.countApproved(eventId, s.id))),
      Promise.all(slots.map((s) => this.careerSlotsRepository.findRequestByRequester(eventId, s.id, callerUid))),
    ]);
  ...
  async createSlot(eventId: string, dto: CreateCareerSlotDto, createdByUid: string): Promise<void> {
  ...
  async updateSlot(eventId: string, slotId: string, dto: UpdateCareerSlotDto, callerUid: string): Promise<void> {
    ...
    const isAdminOrOrganizer =
      caller?.role === UserRoleEnum.ADMIN || caller?.role === UserRoleEnum.ORGANIZER;
    if (!isAdminOrOrganizer && slot.createdByUid !== callerUid) {
      throw new ForbiddenException('Only the slot creator can edit this slot');
    }
    const updates: Partial<Pick<CareerSlot, 'title' | 'description' | 'startAt' | 'endAt' | 'location' | 'capacity' | 'requirements'>> = {};
  ...
  async deleteSlot(eventId: string, slotId: string, callerUid: string): Promise<void> {
    const [slot, caller] = await Promise.all([
      this.careerSlotsRepository.findSlotById(eventId, slotId),
      this.usersRepository.findByUid(callerUid),
    ]);
    ...
    const isAdminOrOrganizer =
      caller?.role === UserRoleEnum.ADMIN || caller?.role === UserRoleEnum.ORGANIZER;
    if (!isAdminOrOrganizer && slot.createdByUid !== callerUid) {
      throw new ForbiddenException('Only the slot creator can delete this slot');
    }
    await this.careerSlotsRepository.deleteSlot(eventId, slotId);
  }

  async requestSlot(eventId: string, slotId: string, requesterUid: string, subSlotIndex: number): Promise<void> {
    ...
    const existing = await this.careerSlotsRepository.findRequestByRequester(eventId, slotId, requesterUid);
    if (existing) throw new ConflictException('You already have a request for this slot');
  ...
  async listRequests(eventId: string, slotId: string, callerUid: string): Promise<CareerSlotRequestWithName[]> {
    const [slot, caller] = await Promise.all([
      this.careerSlotsRepository.findSlotById(eventId, slotId),
      this.usersRepository.findByUid(callerUid),
    ]);
    ...
    const requests = await this.careerSlotsRepository.listRequests(eventId, slotId);
    const users = await Promise.all(requests.map((r) => this.usersRepository.findByUid(r.requesterUid)));
  ...
  async respondToRequest(
    ...
    const isAdminOrOrganizer =
      caller?.role === UserRoleEnum.ADMIN || caller?.role === UserRoleEnum.ORGANIZER;
    if (!isAdminOrOrganizer && slot.createdByUid !== callerUid) {
      throw new ForbiddenException('Only the slot creator can respond to requests');
    }

    if (status === 'approved') {
      const [approvedCount, subSlotTaken] = await Promise.all([
        this.careerSlotsRepository.countApproved(eventId, slotId),
        this.careerSlotsRepository.findApprovedBySubSlotIndex(eventId, slotId, request.subSlotIndex),
      ]);
      if (approvedCount >= slot.capacity) throw new ConflictException('This slot is fully booked');
      if (subSlotTaken) throw new ConflictException('This sub-slot is already taken');
    }
    ...
    const requester = await this.usersRepository.findByUid(request.requesterUid);
